...
static void ip_dump(const tunnel_ip_stats *stats, dump_writer writer, void *writer_ctx) {
    ...
    for (i = 0; conns[i] != NULL; i++) {
        snprintf(local_addr, sizeof(local_addr), "%s:%" PRIu64, conns[i]->local_ip, conns[i]->local_port);
        snprintf(remote_addr, sizeof(remote_addr), "%s:%" PRIu64, conns[i]->remote_ip, conns[i]->remote_port);
        writer(writer_ctx, "%-12s%-40s%-40s%-16s%-24s\n",
