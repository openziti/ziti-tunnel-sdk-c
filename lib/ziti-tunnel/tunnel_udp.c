...
u8_t recv_udp(void *tnlr_ctx_arg, struct raw_pcb *pcb, struct pbuf *p, const ip_addr_t *addr) {
    ...
    TNL_LOG(intercept_ctx_get_log_level(intercept_ctx), "intercepted address[%s] client[%s] service[%s]",
            io->tnlr_io->intercepted, io->tnlr_io->client, intercept_ctx->service_name);
