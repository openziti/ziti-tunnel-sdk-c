...
u8_t recv_udp(void *tnlr_ctx_arg, struct raw_pcb *pcb, struct pbuf *p, const ip_addr_t *addr) {
    ...
    if (npcb == NULL) {
        TNL_LOG(ERROR, "unable to allocate UDP pcb - UDP connection limit is %d", MEMP_NUM_UDP_PCB);
        pbuf_free(p);
        return 1;
    }
    ...
    if (err != ERR_OK) {
        TNL_LOG(ERROR, "failed to udp_connect %s:%d: err: %d", ipaddr_ntoa(&src), src_p, err);
        udp_remove(npcb);
        pbuf_free(p);
        return 1;
    }
