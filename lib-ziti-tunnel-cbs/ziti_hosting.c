...
static void ziti_conn_close_cb(ziti_connection zc) {
    ...
    if (io_ctx) {
        TNL_LOG(TRACE, "hosted_service[%s] client[%s] ziti_conn[%p] closed",
...
static void free_hosted_service_ctx(struct hosted_service_ctx_s *hosted_ctx) {
    ...
    switch (hosted_ctx->cfg_type) {
        ...
        case SERVER_CFG_V1:
            ...
            break;
        default:
            TNL_LOG(DEBUG, "unexpected cfg_type %d", hosted_ctx->cfg_type);
            break;
    }
}
...
static void hosted_server_close_cb(uv_handle_t *handle) {
    ...
    if (io_ctx->client) {
        ...
        TNL_LOG(TRACE, "hosted_service[%s] client[%s] server_conn[%p] closed",
                io_ctx->service->service_name, ziti_conn_source_identity(io_ctx->client), handle);
    } else {
        TNL_LOG(TRACE, "server_conn[%p] closed", handle);
...
static ssize_t on_hosted_client_data(ziti_connection clt, uint8_t *data, ssize_t len) {
    ...
    if (io_ctx == NULL) {
        TNL_LOG(DEBUG, "null io");
        if (len > 0) {
            TNL_LOG(DEBUG, "closing ziti connection");
    ...
    if (len > 0) {
        ...
        switch (io_ctx->server_proto_id) {
            case IPPROTO_TCP: {
                ...
                if (err < 0) {
                    TNL_LOG(ERROR, "uv_write failed: %s", uv_err_name(err));
            ...
            case IPPROTO_UDP: {
                ...
                if (err < 0) {
                    TNL_LOG(ERROR, "uv_udp_send failed: %s", uv_err_name(err));
                    on_hosted_udp_client_write(req, err);
                }
            }
                break;
            default:
                TNL_LOG(ERROR, "invalid protocol %d in server config for service %s", io_ctx->server_proto_id, io_ctx->service->service_name);
                break;
        }
    }
    else if (len == ZITI_EOF) {
        // client will not send more data, but should send one more message for connection closed.
        TNL_LOG(DEBUG, "hosted_service[%s] client[%s] sent EOF, ziti_eof=%d, tcp_eof=%d", io_ctx->service->service_name,
        ...
        TNL_LOG(DEBUG, "client status %s. closing server connection", ziti_errorstr(len));
        hosted_server_close(io_ctx);
    }
    return len;
}
...
static void on_hosted_tcp_server_data(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf) {
    ...
    if (nread > 0) {
        ...
        if (zs != ZITI_OK) {
            TNL_LOG(ERROR, "ziti_write to %s failed: %s", ziti_conn_source_identity(io_ctx->client),
                    ziti_errorstr(zs));
        ...
        if (nread == UV_ENOBUFS) {
            TNL_LOG(WARN, "tcp server is throttled: could not allocate buffer for incoming data [%zd](%s)", nread, uv_strerror(nread));
        } else if (nread == UV_EOF) {
            TNL_LOG(DEBUG, "server sent FIN ziti_eof=%d, tcp_eof=%d, io=%p", io_ctx->ziti_eof, io_ctx->tcp_eof, io_ctx);
            io_ctx->tcp_eof = true;
            if (io_ctx->ziti_eof) {
            ...
            TNL_LOG(WARN, "error reading from server [%zd](%s)", nread, uv_strerror(nread));
            hosted_server_close(io_ctx);
        }
...
static void on_hosted_udp_server_data(uv_udp_t* handle, ssize_t nread, const uv_buf_t* buf, const struct sockaddr* addr, unsigned flags) {
    ...
    if (nread > 0) {
        int zs = ziti_write(io_ctx->client, buf->base, nread, on_hosted_ziti_write, buf->base);
        if (zs != ZITI_OK) {
            TNL_LOG(ERROR, "ziti_write failed: %s", ziti_errorstr(zs));
            on_hosted_ziti_write(io_ctx->client, nread, buf->base);
            hosted_server_close(io_ctx);
        }
        ...
        TNL_LOG(ERROR, "error receiving data from hosted service %s", io_ctx->service->service_name);
        hosted_server_close(io_ctx);
    }
}
...
static void on_hosted_client_connect_complete(ziti_connection clt, int err) {
    struct hosted_io_ctx_s *io_ctx = ziti_conn_data(clt);
    if (err == ZITI_OK) {
        TNL_LOG(DEBUG, "hosted_service[%s] client[%s] connected", io_ctx->service->service_name, ziti_conn_source_identity(clt));
        switch (io_ctx->server_proto_id) {
            case IPPROTO_TCP:
        ...
        TNL_LOG(ERROR, "hosted_service[%s] client[%s] failed to connect: %s", io_ctx->service->service_name,
...
static void on_hosted_tcp_server_connect_complete(uv_connect_t *c, int status) {
    if (c == NULL || c->handle == NULL || c->handle->data == NULL) {
        TNL_LOG(ERROR, "null handle or io_ctx");
    }
    struct hosted_io_ctx_s *io_ctx = c->handle->data;
    if (io_ctx->client == NULL) {
        TNL_LOG(ERROR, "client closed before server connection was established");
        hosted_server_close(io_ctx);
        free(c);
        return;
    }
    ...
    if (status < 0) {
        TNL_LOG(ERROR, "hosted_service[%s], client[%s]: connect to %s failed: %s", io_ctx->service->service_name,
                ziti_conn_source_identity(io_ctx->client), io_ctx->server_dial_str, uv_strerror(status));
        hosted_server_close(io_ctx);
        free(c);
        return;
    }
    TNL_LOG(DEBUG, "hosted_service[%s], client[%s]: connected to server %s", io_ctx->service->service_name,
...
static void on_hosted_client_connect(ziti_connection serv, ziti_connection clt, int status, ziti_client_ctx *clt_ctx) {
    ...
    if (service_ctx == NULL) {
        TNL_LOG(ERROR, "null service_ctx");
        ziti_close(clt, ziti_conn_close_cb);
        return;
    }

    if (status != ZITI_OK) {
        TNL_LOG(ERROR, "incoming connection to service[%s] failed: %s", service_ctx->service_name, ziti_errorstr(status));
        ziti_close(clt, ziti_conn_close_cb);
        return;
    }
    ...
    if (clt_ctx->app_data != NULL) {
        TNL_LOG(DEBUG, "hosted_service[%s], client[%s]: received app_data_json='%.*s'", service_ctx->service_name,
        ...
        if (parse_tunneler_app_data(&app_data_model, (char *)clt_ctx->app_data, clt_ctx->app_data_sz) != 0) {
            TNL_LOG(ERROR, "hosted_service[%s], client[%s]: failed to parse app_data_json '%.*s'",
    ...
    switch (service_ctx->cfg_type) {
        ...
        case SERVER_CFG_V1:
            ...
            break;
        default:
            TNL_LOG(ERROR, "hosted_service[%s], client[%s]: unexpected cfg_type %d",
    ...
    if (!s) {
        TNL_LOG(ERROR, "hosted_service[%s], client[%s]: failed to create dial addrinfo params: %s",
    ...
    if ((s = getaddrinfo(dial_ai_params.address, dial_ai_params.port, &dial_ai_params.hints, &dial_ai)) != 0) {
        TNL_LOG(ERROR, "hosted_service[%s], client[%s]: getaddrinfo(%s,%s) failed: %s",
    ...
    if (dial_ai->ai_next != NULL) {
        TNL_LOG(DEBUG, "hosted_service[%s], client[%s]: getaddrinfo(%s,%s) returned multiple results; using first",
    ...
    if (iproto != NULL && iip != NULL && iport != NULL) {
        TNL_LOG(INFO, "hosted_service[%s], client[%s] intercepted_addr[%s:%s:%s]: incoming connection",
                service_ctx->service_name, client_identity, iproto, iip, iport);
    } else {
        TNL_LOG(INFO, "hosted_service[%s], client[%s] incoming connection",
    ...
    if (s == 0) {
        ...
        TNL_LOG(WARN, "hosted_service[%s] client[%s] getnameinfo failed: %s", io_ctx->service->service_name,
    ...
    switch (dial_ai->ai_protocol) {
        case IPPROTO_TCP:
            ...
            if (source_ai != NULL) {
                ...
                if (uv_err != 0) {
                    TNL_LOG(ERROR, "hosted_service[%s], client[%s]: uv_tcp_bind failed: %s",
            ...
            }
            {
                ...
                if (uv_err != 0) {
                    TNL_LOG(ERROR, "hosted_service[%s], client[%s]: uv_tcp_connect failed: %s",
                            service_ctx->service_name, client_identity, uv_err_name(uv_err));
                    err = true;
                    goto done;
                }
        ...
        case IPPROTO_UDP:
            ...
            if (source_ai != NULL) {
                ...
                if (uv_err != 0) {
                    TNL_LOG(ERROR, "hosted_service[%s] client[%s]: uv_udp_bind failed: %s",
                            service_ctx->service_name, client_identity, uv_err_name(uv_err));
                    err = true;
                    goto done;
                }
            ...
            if (uv_err != 0) {
                TNL_LOG(ERROR, "hosted_service[%s], client[%s]: uv_udp_connect failed: %s",
                        service_ctx->service_name, client_identity, uv_err_name(uv_err));
                err = true;
                goto done;
            }
            ...
            if (uv_err != 0) {
                TNL_LOG(ERROR, "hosted_service[%s] client[%s]: uv_udp_recv_start failed: %s",
                        service_ctx->service_name, client_identity, uv_err_name(uv_err));
                err = true;
                goto done;
            }
            if (ziti_accept(clt, on_hosted_client_connect_complete, on_hosted_client_data) != ZITI_OK) {
                TNL_LOG(ERROR, "ziti_accept failed");
                err = true;
                goto done;
            }
            break;
...
static void hosted_listen_cb(ziti_connection serv, int status) {
    ...
    if (host_ctx == NULL) {
        TNL_LOG(DEBUG, "null host_ctx");
        return;
    }

    if (status != ZITI_OK) {
        TNL_LOG(ERROR, "unable to host service %s: %s", host_ctx->service_name, ziti_errorstr(status));
...
static void listen_opts_from_host_cfg_v1(ziti_listen_opts *opts, const ziti_host_cfg_v1 *config) {
    ...
    if (t != NULL) {
        if (opts->bind_using_edge_identity) {
            TNL_LOG(WARN, "listen options specifies both 'identity=%s' and 'bindUsingEdgeIdentity=true'",
                    t->string_value);
        } else {
    ...
    if (t != NULL) {
        if (strcmp(t->string_value, "default") == 0) {
            ...
            TNL_LOG(WARN, "unsupported terminator precedence '%s'", t->string_value);
        }
    }
...
host_ctx_t *ziti_sdk_c_host(void *ziti_ctx, uv_loop_t *loop, const char *service_name, cfg_type_e cfg_type, const void *cfg) {
    if (service_name == NULL) {
        TNL_LOG(ERROR, "null service_name");
        return NULL;
    }
    ...
    switch (cfg_type) {
        ...
        case SERVER_CFG_V1: {
            ...
            break;
        default:
            TNL_LOG(WARN, "unexpected cfg_type %d", cfg_type);
            break;
    }
