...
//
// Both prefixes also come back forward-slash-only in the captured template
// on Windows, never matching a filepath.Join-built (backslash-native) search
// string there - not just the PKI cert/key fields (which are explicitly run
// through ziti's helpers.NormalizePath in create_config_controller.go,
// because those values sit inside double-quoted YAML scalars where a raw
// backslash would start an escape sequence), but dataDir too: its value
// comes from cmdHelper.GetZitiHome(), which unconditionally ends with
// `return NormalizePath(retVal)` (env_helpers.go) - easy to miss since nothing
// in the *callers* of GetZitiHome hints at it. Confirmed live on Windows CI
// twice: first the PKI paths, then dataDir, each time leaving a real node's
// config pointing at the already-deleted template-capture directory because
// only the backslash-native replace was attempted. Matching (and replacing
// with) the slash-normalized form for both prefixes fixes this without
// touching non-Windows behavior, where ToSlash is a no-op.
//
// Both substitutions are verified, not blind: a silent no-op match failure
// is exactly how both of the above got past review, and it's cheap to
// confirm the prefix is actually there before betting the rest of the
// config on it. On a miss, this dumps the whole captured template so the
// actual (as opposed to assumed) separator/normalization convention is
// visible directly in the failure, rather than surfacing much later as an
// unrelated timeout with no clue why the node's on-disk paths are wrong.
func (c *HACluster) renderNodeConfig(template, instanceID string, bindPort, proxyPort int, intermediateName string) (string, error) {
	...
	cfg := template
	var err error
	if cfg, err = replacePathPrefix(cfg, templateInstHome, c.Home+"/"+instanceID, "dataDir"); err != nil {
		return "", err
	}
	if cfg, err = replacePathPrefix(cfg, templatePkiDir, c.Home+"/pki", "pki dir"); err != nil {
		return "", err
	}
...
// replacePathPrefix replaces every occurrence of the "from" path prefix in
// cfg with "to", trying both from's native (filepath.Join) form and its
// forward-slash-normalized form - see renderNodeConfig's doc comment for why
// a captured ziti config can contain either, depending on the field. Errors
// out with the full (unmodified) cfg if neither form matches, instead of
// silently leaving the original prefix in place.
func replacePathPrefix(cfg, from, to, label string) (string, error) {
	fromSlash := filepath.ToSlash(from)
	switch {
	case strings.Contains(cfg, fromSlash):
		return strings.ReplaceAll(cfg, fromSlash, filepath.ToSlash(to)), nil
	case strings.Contains(cfg, from):
		return strings.ReplaceAll(cfg, from, to), nil
	default:
		return "", fmt.Errorf("renderNodeConfig: %s prefix %q (native) / %q (slash-normalized) not found in captured template (%d bytes):\n%s",
			label, from, fromSlash, len(cfg), cfg)
	}
}
